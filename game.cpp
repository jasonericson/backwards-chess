...
void highlight_square(uint16 col, uint16 row, bool highlight)
{
    ...
    {
        if (grid[col][row].overlay_sprite == 0)
            grid[col][row].overlay_sprite = sprite_create(&square_tex, grid_pos_x + col * 14, grid_pos_y + row * 14, Layer_SquareHighlight, 1.0f, 1.0f, 0.0f, 1.0f, 1.0f, 1.0f, 0.4f);
    }
    else
    {
        if (grid[col][row].overlay_sprite != 0)
            ...
            case Action_GridSquare:
                {
                    ...
                    if (held_piece.type != Piece_None)
                    {
                        ...
                        else if (is_valid_space(grid_sq->col, grid_sq->row))
                        {
                            ...
                            // show piece preview
                            if (preview_sprite == 0)
                                preview_sprite = sprite_create(piece_textures[held_piece.type - 1], grid_pos_x + grid_sq->col * 14, grid_pos_y + grid_sq->row * 14, Layer_PlacedPiece, 1.0f, 1.0f, 0.0f, 1.0f, 1.0f, 1.0f, 0.5f);
                            else
